...
pub(crate) fn get_top_n(limit: i64, conn: &mut MysqlConnection) -> Result<Vec<DointUser>, diesel::result::Error> {
    // Transaction not really needed here but lol.
    // Users table, ordered by balance, limited.
    conn.transaction(|conn|{
        users.order_by(bal.desc()).limit(limit).load::<DointUser>(conn)
    })
}