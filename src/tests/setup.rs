...
use diesel::{r2d2::{self, ConnectionManager}, Connection, MysqlConnection};
use dotenvy::dotenv;

/// You need to call TEST transactions on this or you will be altering the actual DB!
pub(super) fn get_test_db() -> diesel::r2d2::PooledConnection<diesel::r2d2::ConnectionManager<diesel::MysqlConnection>> {
    ...
    let db_pool = r2d2::Pool::builder().build(manager).expect("Failed to create DB pool!");

    // Now we obviously dont want the DB to be live, so we immediately go into a test db state.
    db_pool.get().expect("Opening the DB failed. Test failure non-test related.")
}
