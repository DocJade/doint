...
pub(in super::super) async fn handle_error(error: poise::FrameworkError<'_, Data, Error>) {
    match error {
        poise::FrameworkError::Setup { error, framework, data_about_bot, ctx, ..} => {
        ...
        poise::FrameworkError::CommandCheckFailed { error, ctx, .. } => {
            handle_command_check_failed_error(error.expect("Failed command checks always have an error."), ctx).await;
        },
        poise::FrameworkError::DynamicPrefix { error, ctx, msg, .. } => {
            handle_dynamic_prefix_error(error, ctx, msg).await;
        },
        poise::FrameworkError::UnknownCommand { ctx, msg, prefix, msg_content, framework, invocation_data, trigger, ..} => {
            handle_unknown_command_error(ctx, msg, prefix, msg_content, framework, invocation_data, trigger).await;
        },
        poise::FrameworkError::UnknownInteraction { ctx, framework, interaction, ..} => {
...
/// 
/// Returns nothing.
fn handle_setup_error(error: &DointBotError, _framework: &Framework<Data, DointBotError>, _data_about_bot: &Ready, _ctx: &poise::serenity_prelude::Context) {
    // This error was thrown during bot startup. We cannot recover from this cleanly, we'll shut down.
    error!("Creating user data on bot startup failed! \n\n{error}\n\nWe cannot recover from this state! Shutting down.");
    exit(-1);
}
...
/// 
/// Returns nothing.
fn handle_event_handler_error(error: &DointBotError, _ctx: &poise::serenity_prelude::client::Context, _event: &FullEvent, _framework: poise::FrameworkContext<'_, Data, DointBotError>) {
    // todo
...
/// 
/// Returns nothing.
async fn handle_cooldown_hit_error(remaining_cooldown: Duration, ctx: Context<'_, Data, DointBotError>) {
    ...
    let _ = ctx.defer_ephemeral().await;
    let _ = ctx.say(format!("You're on cooldown! You can run this command <t:{end_unix_time}:R>.")).await;
}
...
/// 
/// Returns nothing.
async fn handle_missing_bot_permissions_error(missing_permissions: Permissions, ctx: Context<'_, Data, DointBotError>) {
    ...
    let _ = ctx.say("Bot doesn't have perms for that command? This shouldn't happen, tell Doc!").await;
    // TODO: Admin warning
...
/// 
/// Returns nothing.
async fn handle_missing_user_permissions_error(missing_permissions: Option<Permissions>, ctx: Context<'_, Data, DointBotError>) {
    ...
    let _ = ctx.say("You're not allowed to do that! TELL DOC IMMEDIATELY!").await;
}
...
/// 
/// Returns nothing.
async fn handle_command_check_failed_error(error: DointBotError, ctx: Context<'_, Data, DointBotError>) {
...
/// 
/// Returns nothing.
async fn handle_dynamic_prefix_error(error: DointBotError, ctx: poise::PartialContext<'_, Data, DointBotError>, msg: &Message) {
    // Not aware of us ever using dynamic prefixes.
    error!("Dynamic prefix called! We don't use those! {error}");
    let _ = msg.reply(ctx.serenity_context, "Dynamic prefix? What??? Tell Doc IMMEDIATELY!").await;
}
...
/// 
/// Returns nothing.
async fn handle_unknown_command_error(ctx: &poise::serenity_prelude::Context, msg: &Message, prefix: &str, msg_content: &str, _framework: poise::FrameworkContext<'_, Data, DointBotError>, _invocation_data: &Mutex<Box<dyn Any + Send + Sync>>, _trigger: poise::MessageDispatchTrigger) {
    ...
    let _ = msg.reply(ctx, "??? Why are prefix commands enabled? Tell Doc IMMEDIATELY!").await;
}
...
/// 
/// Returns nothing.
fn handle_unknown_interaction_error(_ctx: &poise::serenity_prelude::Context, _framework: poise::FrameworkContext<'_, Data, DointBotError>, interaction: &poise::serenity_prelude::CommandInteraction) {
...
async fn actually_handle_command_check_failure(ctx: Context<'_, Data, DointBotError>, failure: CommandCheckFailureReason) {
    match failure {
        CommandCheckFailureReason::UserNotEnrolled => {
            let _ = ctx.defer_ephemeral().await;
            let _ = ctx.say("You aren't enrolled in Doints! Run `/opt_in` to join!").await;
        },
        ...
        CommandCheckFailureReason::InvalidChannel => {
            let _ = ctx.defer_ephemeral().await;
            let _ = ctx.say("You can't run that in here! Go run this command in the appropriate channel.").await;
        },
        ...
        CommandCheckFailureReason::UserInJail(jailed_user) => {
            let _ = ctx.defer_ephemeral().await;
            let _ = ctx.say(format!("You're in jail! You'l be released <t:{}:R>.", jailed_user.until.and_utc().timestamp())).await;
        },
    }
}